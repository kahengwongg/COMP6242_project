"""
M4: Combined Model (WGAN-GP + Self-Attention)
- Architecture: M2 (WGAN-GP) + M3 (Self-Attention)
- Self-Attention inserted after the second-to-last layer in G and D
...
from .layers import ConvTransposeBlock, LayerNormConvBlock, SelfAttention, weights_init


class CombinedGenerator(nn.Module):
    """
    Combined Generator: WGAN-GP + Self-Attention
...
class CombinedDiscriminator(nn.Module):
    """
    Combined Discriminator (Critic): WGAN-GP + Self-Attention
    
    Architecture:
        Conv -> Conv -> Conv -> SelfAttention -> Conv -> Output (no Sigmoid)
...
def compute_gradient_penalty(discriminator, real_images, fake_images, device='cuda'):
    ...
    Reused from WGAN-GP implementation
...
def test():
    ...
    # Create models
    G = CombinedGenerator(z_dim=z_dim).to(device)
    D = CombinedDiscriminator().to(device)
